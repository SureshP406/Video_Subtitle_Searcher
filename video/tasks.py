...
def process_video(video_id):
    ...
    s3_key = f'videos/{video.video_file.name}'
    ...
    processed_video_key = f'processed_videos/{video.video_file.name}'
    ...
    with open(subtitle_file, 'r') as f:
        keywords = f.read()
        start_time = video.subtitle.start
        end_time = video.subtitle.end
    ...
    video = Video.objects.create(video_file=video.video_file,processed_file = processed_video_key, subtitles=keywords, start_time=start_time, end_time=end_time)
    ...
    # video.save()
    return redirect('processing.html', {'processed_video_url': processed_video_url})